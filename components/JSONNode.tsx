...
import { PlusCircle, Trash2, ChevronDown, ChevronRight, Type, List, Box } from 'lucide-react'
...
export function JSONNode({ 
  ...
  const nodeLabel = parentType === 'array' ? `Item ${index! + 1}` : node.key || 'Root'

  return (
    <div className="border rounded-lg my-2 bg-background shadow-sm" 
      ...
      <div className="p-3">
        <div className="flex items-center gap-2 mb-2">
          ...
          <div className="flex items-center gap-2 flex-1">
            <Input 
              ...
              placeholder={isRoot ? 'Root Key (optional)' : parentType === 'array' ? nodeLabel : 'Key'}
        ...
        {node.type === 'array' && (
          <div className="mt-2">
            <label className="text-sm font-medium">Number of items to generate: {arrayCount}</label>
            <Slider
              value={[arrayCount]}
      ...
      {isExpanded && canHaveChildren && (
        <div className="px-3 pb-3">
          {node.type === 'array' && Array.isArray(node.value) && node.value.length === 0 && (
