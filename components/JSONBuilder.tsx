...
export function JSONBuilder({ onUpdate, initialData }: JSONBuilderProps) {
  ...
  useEffect(() => {
    if (initialData) {
      const initialNodes = Object.entries(initialData as JSONObject).map(([key, value]) => ({
        key,
  ...
  const nodeToJSON = (node: JSONNode): JSONValue => {
    if (node.type === 'object') {
      return Object.entries(node.value as JSONObject).reduce((acc, [key, value]) => {
    ...
    setNodes(prevNodes => {
      ...
      for (let i = 1; i < path.length - 1; i++) {
        if (currentNode.type === 'object') {
          currentNode = Object.values(currentNode.value as JSONObject)[path[i]] as JSONNode
  ...
  const addChildNode = (parentIndex: number[]) => {
    setNodes(prevNodes => {
      ...
      for (let i = 1; i < parentIndex.length; i++) {
        if (currentNode.type === 'object') {
          currentNode = Object.values(currentNode.value as JSONObject)[parentIndex[i]] as JSONNode
        } else if (currentNode.type === 'array') {
          currentNode = (currentNode.value as JSONArray)[parentIndex[i]] as JSONNode
        }
      }

      if (currentNode.type === 'object') {
        const tempKey = `field_${Object.keys(currentNode.value as JSONObject).length}`
        const updatedValue = {
  ...
  return (
    <div className="p-4 h-full overflow-auto">
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-xl font-bold">JSON Builder</h2>
